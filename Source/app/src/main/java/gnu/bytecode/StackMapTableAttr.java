...
public class StackMapTableAttr extends MiscAttr {
    ...
    public void emitStackMapEntry(Label label, CodeAttr code) {
        ...
        } else if (compressStackMapTable && curStackCount == 0 && curLocalsCount3 < this.countLocals && unchangedLocals == curLocalsCount3 && localsDelta >= -3) {
            put1(localsDelta + Telnet.WILL);
            put2(offset_delta);
        } else if (compressStackMapTable && curStackCount == 0 && this.countLocals == unchangedLocals && localsDelta <= 3) {
            put1(localsDelta + Telnet.WILL);
            put2(offset_delta);
            for (int i4 = 0; i4 < localsDelta; i4++) {
                emitVerificationType(this.encodedLocals[unchangedLocals + i4]);
            }
        } else {
            put1(255);
            put2(offset_delta);
            put2(curLocalsCount3);
            for (int i5 = 0; i5 < curLocalsCount3; i5++) {
    ...
    public void print(ClassTypeWriter dst) {
        ...
        for (int i = 0; i < this.numEntries && ipos < this.dataLength; i++) {
            int ipos2 = ipos + 1;
            int tag = u1(ipos);
            ...
            if (tag <= 63) {
                ...
                curLocals += count2;
            } else if (ipos + 1 < this.dataLength) {
