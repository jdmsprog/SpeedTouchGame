...
public abstract class RatNum extends RealNum {
    ...
    public static RatNum valueOf(BigDecimal value) {
        ...
        if (scale > 0) {
            return divide(v, (RatNum) scaleVal);
        }
        return times(v, (RatNum) scaleVal);
    }
    ...
    public static RatNum add(RatNum x, RatNum y, int k) {
        ...
        return IntNum.equals(x_den, y_den) ? make(IntNum.add(x_num, y_num, k), x_den) : make(IntNum.add(IntNum.times(y_den, x_num), IntNum.times(y_num, x_den), k), IntNum.times(x_den, y_den));
    }
