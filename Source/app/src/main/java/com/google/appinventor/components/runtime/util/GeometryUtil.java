...
public final class GeometryUtil {
    ...
    private static final GeometryFactory FACTORY = new GeometryFactory(new PrecisionModel(), (int) WEB_MERCATOR_SRID);
    ...
    public static Geometry createGeometry(List<List<GeoPoint>> points, List<List<List<GeoPoint>>> holes) {
        ...
        if (holes == null || holes.isEmpty()) {
            ...
            for (List<GeoPoint> list : points) {
                geometryArr[i] = ringToPolygon(list, jp.next());
                i++;
            }
        }
    ...
    public static double distanceBetween(IGeoPoint a, IGeoPoint b) {
        ...
        double angle = 2.0d * Math.atan2(Math.sqrt(cordlen), Math.sqrt(1.0d - cordlen));
